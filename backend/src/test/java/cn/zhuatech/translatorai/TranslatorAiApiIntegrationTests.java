 ...
 */
@SpringBootTest @AutoConfigureMockMvc class TranslatorAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanCreateReviewPlan()throws Exception{mvc.perform(post("/api/ai/translation/review-plan").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"documentNo\":\"DOC-88\",\"sourceLanguage\":\"zh-CN\",\"targetLanguage\":\"en-US\",\"wordCount\":9000,\"glossaryCoveragePercent\":60,\"modelConfidencePercent\":64,\"layoutComplexity\":4,\"confidential\":true,\"legalDocument\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.reviewRoute").value("SENIOR_REVIEW"));}}
